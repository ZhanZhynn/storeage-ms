...
const emptyQuote = { supplierName: "", unitPriceRmb: 0, samplePhotoUrls: [], certifications: [], priceBreaks: [] };
...
function quoteValues(quote: any): SourcingQuoteInput {
  return {
    ...
    paymentTerms: quote.paymentTerms, certifications: Array.isArray(quote.certifications) ? quote.certifications : [],
    complianceNotes: quote.complianceNotes, riskLevel: quote.riskLevel, recommendation: quote.recommendation,
...
export default function SourcingCaseDetail({ caseId, basePath = "/sourcing" }: { caseId: string; basePath?: string }) {
  ...
  const [insuranceMyr, setInsuranceMyr] = useState("0");
  const [localDeliveryMyr, setLocalDeliveryMyr] = useState("0");
  ...
  return <main className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
    ...
    {offers.length > 0 && <Card><CardHeader><CardTitle>Supplier offer comparison</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">{offers.map((quote: any) => <button key={offerKey(quote)} type="button" onClick={() => chooseQuote(quote)} className={`rounded-lg border p-4 text-left text-sm ${activeQuoteId === quote.id ? "border-sky-500 ring-1 ring-sky-500" : "hover:bg-muted/50"}`}><div className="flex justify-between gap-2"><b>{quote.supplierName}</b><span className="capitalize text-muted-foreground">{label(quote.status)}</span></div><p className="mt-2">{quote.unitPriceRmb == null ? "No price" : formatMoney(quote.unitPriceRmb, "CNY")} / unit</p><p>MOQ: {quote.moq ?? "-"} | Lead time: {quote.leadTimeDays ?? "-"} days</p><p>Payment: {quote.paymentTerms || "-"} | Risk: {quote.riskLevel || "-"}</p>{Array.isArray(quote.certifications) && quote.certifications.length > 0 && <p>Compliance: {quote.certifications.join(", ")}</p>}{Array.isArray(quote.priceBreaks) && quote.priceBreaks.length > 0 && <p>Price breaks: {quote.priceBreaks.map((breakpoint: any) => `${breakpoint.minQuantity}+: ${formatMoney(breakpoint.unitPriceRmb, "CNY")}`).join(" | ")}</p>}<p className="mt-1 text-xs text-muted-foreground">Revision {quote.revision}{item.selectedQuoteId === quote.id ? " | Approved selection" : ""}</p></button>)}</CardContent></Card>}
    {activeQuote?.unitPriceRmb != null && <Card><CardHeader><CardTitle>Landed-cost estimate</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-4"><label className="grid gap-1 text-sm font-medium">Freight (RM)<Input type="number" min="0" value={freightMyr} onChange={(event) => setFreightMyr(event.target.value)} /></label><label className="grid gap-1 text-sm font-medium">Duty %<Input type="number" min="0" max="100" value={dutyRate} onChange={(event) => setDutyRate(event.target.value)} /></label><label className="grid gap-1 text-sm font-medium">Tax %<Input type="number" min="0" max="100" value={taxRate} onChange={(event) => setTaxRate(event.target.value)} /></label><label className="grid gap-1 text-sm font-medium">Insurance (RM)<Input type="number" min="0" value={insuranceMyr} onChange={(event) => setInsuranceMyr(event.target.value)} /></label><label className="grid gap-1 text-sm font-medium">Local delivery (RM)<Input type="number" min="0" value={localDeliveryMyr} onChange={(event) => setLocalDeliveryMyr(event.target.value)} /></label><div className="flex items-end"><Button isLoading={landedCost.isPending} onClick={() => landedCost.mutate({ quantity: item.requestedQuantity || activeQuote.moq || 1, unitPriceCny: activeQuote.unitPriceRmb, fxRate: activeQuote.fxRate || 0.65, freightMyr: Number(freightMyr) || 0, dutyRate: Number(dutyRate) || 0, taxRate: Number(taxRate) || 0, insuranceMyr: Number(insuranceMyr) || 0, localDeliveryMyr: Number(localDeliveryMyr) || 0 })}>Calculate</Button></div>{landedCost.data && <p className="text-sm sm:col-span-4">Goods {formatMoney(landedCost.data.goodsMyr, "MYR")} | Freight {formatMoney(landedCost.data.freightMyr, "MYR")} | Duty {formatMoney(landedCost.data.dutyMyr, "MYR")} | Tax {formatMoney(landedCost.data.taxMyr, "MYR")} | Insurance {formatMoney(landedCost.data.insuranceMyr, "MYR")} | Local delivery {formatMoney(landedCost.data.localDeliveryMyr, "MYR")}<br />Estimated total: <b>{formatMoney(landedCost.data.totalMyr, "MYR")}</b> | Unit landed cost: <b>{formatMoney(landedCost.data.unitLandedMyr, "MYR")}</b> <span className="text-muted-foreground">(estimate only; it does not change the quote or PO)</span></p>}</CardContent></Card>}
