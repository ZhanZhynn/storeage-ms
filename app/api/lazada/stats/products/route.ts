/**
 * Lazada Product Performance — Stock turnover, days-until-stockout, low-stock alerts
 * GET /api/lazada/stats/products
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `lazada:product-performance:${shopId || "all"}`;
    ...
    const products = await prisma.lazadaProduct.findMany({
    ...
    const recentOrderItems = await prisma.lazadaOrderItem.findMany({
      where: {
        order: {
          shopId: { in: shopIds },
          lazadaCreatedAt: { gte: thirtyDaysAgo },
    ...
    for (const item of recentOrderItems) {
      ...
      if (velocity) {
        ...
        velocity.revenue += item.price * item.quantity;
        velocity.orderCount++;
      }
    }
    ...
    logger.error("[Lazada Product Performance] Error:", error);
